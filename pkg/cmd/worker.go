...
func work(options *Options) {
	...
	for {
		...
		for i := range vsList.Items {
			...
			if generated == "true" {
				...
				for _, dest := range vs.Spec.GetHttp()[0].GetRoute() {
					...
					if dh == options.InternalDestinationHost {
						internalWeight = int(dw)
					} else if dh == options.BufferDestinationHost {
						bufferWeight = int(dw)
					}
				}
			}
		...
		targetHost = requestCounts.MaxHost
		targetRequestCount := requestCounts.GetCounts(targetHost)
